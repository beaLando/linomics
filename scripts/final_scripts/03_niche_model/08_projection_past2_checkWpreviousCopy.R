...
## CALCULATE AND PLOT CLIMATE STABILITY
gam_proj.lb.mskrd.rs <- resample(gam_proj.lb.mskrd, gam_proj_epli.mskrd)
#gam_proj.lb.mskrd.rs[gam_proj.lb.mskrd.rs < 0.1] <- NA

stack.r <- stack((gam_proj_epli.mskrd),
                 (gam_proj_lpli.mskrd),
                 (gam_proj_eple.mskrd),
                 (gam_proj_mple.mskrd),
                 (gam_proj_lig.mskrd),
                 ...
                 (gam_proj_lgm.mskrd),
                 (gam_proj_hol.mskrd),
                 (gam_proj.lb.mskrd.rs))

mean_stack <- calc(stack.r, fun = function(x) mean(x))
plot(mean_stack)
mean_stack.msk <- mean_stack
mean_stack.msk[mean_stack.msk < 0.1] <- NA
# plot(mean_stack)

sd_stack <- calc(stack.r, fun = function(x) sd(x)) #in this case I prefer SD to CV
plot(sd_stack)

sd_stack.msk <- sd_stack
sd_stack.msk[mean_stack < 0.1] <- NA

cv_stack <- calc(stack.r, fun = function(x) sd(x) / mean(x))
plot(cv_stack)
cv_stack[mean_stack < 1/2] <- NA
plot(cv_stack)

# stack.r2 <- stack.r
# stack.r2[mean_stack < 0.1] <- NA
# cv_stack <- calc(stack.r2, fun = function(x) sd(x) / mean(x))
# plot(cv_stack)
...
writeRaster(gam_proj_midh, "./data/niche_model/gam_preds_lb_9_SD.tif", overwrite = TRUE)
...
#### Mean
gam_proj.df <- as.data.frame(as(mean_stack, "SpatialPixelsDataFrame"))

mean_stack.msk2 <- mean_stack.msk
mean_stack.msk2[mean_stack.msk2 <= 0.1] <- NA 
mean_stack.msk2[mean_stack.msk2 > 0.1] <- 1 

mean_stack.msk2 <- terra::as.polygons(terra::rast(mean_stack.msk2))
mean_stack.msk2 <- sf::st_as_sf(mean_stack.msk2)

mapMN <- eu_map +
  geom_tile(data = gam_proj.df, aes(x = x, y = y, fill = layer), alpha = 0.8) + #0.18 is 85% quantile for SD, excluding areas where favourability < 0.1
  borders("world", col = "black") +
  geom_sf(data = mean_stack.msk2, 
          fill = "transparent", col = "darkgreen") +
  coord_sf(default_crs = sf::st_crs(4326), xlim = c(-20,60), ylim = c(25,60)) +
  scale_fill_gradient(low = "white", high = "black", na.value = "transparent", limits=c(0,1)) +
  labs(x = "Longitude", y = "Latitude", fill = "Mean Favourability in Time", col = "Mean Favourability > 0.1") +
  theme_minimal() +
  theme(legend.position = c("right"))

#### SD
gam_proj.df <- as.data.frame(as(sd_stack, "SpatialPixelsDataFrame")) %>% 
  left_join(as.data.frame(as(sd_stack.msk, "SpatialPixelsDataFrame")),
            by = c("x", "y")) %>%
  as.data.frame() #%>% str()

##### Check if considering whole area or only area where favourability > 0.1, what is SD quantile
quantile(gam_proj.df$layer.x, probs = c(0, 0.25, 0.5, 0.75, 0.85, 0.95, 1)) #95%
quantile(gam_proj.df$layer.y, probs = c(0, 0.25, 0.5, 0.75, 0.85, 0.95, 1), na.rm = TRUE) #85%

sum(!is.na(gam_proj.df$layer.x))
sum(gam_proj.df$layer.x > 0.18)

sum(!is.na(gam_proj.df$layer.y))
sum(gam_proj.df$layer.y > 0.18, na.rm = TRUE)

##### Plot
sd_stack.msk2 <- sd_stack.msk
sd_stack.msk2[sd_stack.msk2 <= 0.18] <- NA 
sd_stack.msk2[sd_stack.msk2 > 0.18] <- 1 

sd_stack.msk2 <- terra::as.polygons(terra::rast(sd_stack.msk2))
sd_stack.msk2 <- sf::st_as_sf(sd_stack.msk2)

sd_stack.msk3 <- sd_stack.msk
sd_stack.msk3[sd_stack.msk3 <= 0.18] <- NA 
sd_stack.msk3[sd_stack.msk3 > 0.18] <- 1 

sd_stack.msk3 <- terra::as.polygons(terra::rast(sd_stack.msk3))
sd_stack.msk3 <- sf::st_as_sf(sd_stack.msk3)

mapSD <- eu_map +
  geom_tile(data = gam_proj.df, aes(x = x, y = y, fill = layer.x), alpha = 0.8) + #0.18 is 85% quantile for SD, excluding areas where favourability < 0.1
  borders("world", col = "black") +
  geom_sf(data = sd_stack.msk2, fill = NA, aes(colour = "red"), col = "red") +
  coord_sf(default_crs = sf::st_crs(4326), xlim = c(-20,60), ylim = c(25,60)) +
  scale_fill_gradient(low = "white", high = "black", na.value = "transparent", limits=c(0,0.3)) +
  labs(x = "Longitude", y = "Latitude", fill = "Favourability (SD)", col = "Favourability > 0.1, SD > 0.18") +
  theme_minimal() +
  theme(legend.position = c("right"))

#### Mean and SD
mapMNSD <- eu_map +
  geom_sf(data = smoothr::smooth(sf::st_buffer(mean_stack.msk2, dist = 0.001), method = "ksmooth", smoothness = 3, n = 50L), 
          fill = "darkgreen", col = "transparent", alpha = 0.2) +
  geom_sf(data = smoothr::smooth(sf::st_buffer(sd_stack.msk2, dist = 0.001), method = "ksmooth", smoothness = 3, n = 50L), 
          fill = "black", col = "transparent", alpha = 0.3) +
  borders("world", col = "black") +
  coord_sf(default_crs = sf::st_crs(4326), xlim = c(-20,60), ylim = c(25,60)) +
  scale_fill_gradient(low = "white", high = "black", na.value = "transparent", limits=c(0,1)) +
...
mapp <- eu_map +
  ...
  borders("world", col = "black") +
  geom_sf(data = sd_stack.msk2, fill = NA, col = NA) +
  scale_fill_gradient(low = "white", high = "black", na.value = "transparent", limits=c(0,1)) +
  theme_minimal() +
  coord_sf(default_crs = sf::st_crs(4326), xlim = c(-20,60), ylim = c(25,60)) +
...
### ALL TOGETHER
library(patchwork)
(mapp + maph + maplgm + mapali + maplig + mapmple + mapeple + maplpli + mapepli) + plot_layout(guides="collect", ncol = 3, nrow = 3)
# 
# mapp
# maph
# maplgm
